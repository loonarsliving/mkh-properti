-- ============================================================================
-- MKH Property — 0024: Super Admin can reject an approved-but-unpaid pengajuan
--
-- Owner's ask: once a Kepala Cabang approves a gaji tukang/pembelian bahan
-- pengajuan, there was no way to catch a data-entry mistake (e.g. wrong
-- blok) before money moves -- the only path forward was Super Admin sending
-- a bukti transfer, or a manual database fix. MK Connect now supports
-- Super Admin replying "TOLAK <kode>" on WhatsApp (see
-- lib/ai/domains/transfer-rejection.ts), which enqueues this new inbound
-- event instead of finance_expense_transfer_confirmed.
--
-- Sets the pengajuan back to 'rejected' (same status Kepala Cabang's own
-- reject button in verifikasi.html uses) so the submitter has to resubmit
-- it correctly -- nothing is posted to jurnal, mirroring
-- finance_expense_transfer_confirmed's own guard that the pengajuan must
-- still be 'approved' when this event lands.
-- ============================================================================
