-- ============================================================================
-- MKH Property — 0016: Receive construction_expense_submitted from MK Connect
--
-- New sync event from MK Connect's construction project finance module
-- (0193/0194 there): a Kepala Cabang's gaji tukang or pembelian material
-- (utang toko) input. Lands in pengajuan exactly like a project admin
-- entering it by hand via admin-proyek.html -- tipe='tukang' or 'bahan',
-- status='pending', same CFO approval gate as every other expense.
--
-- MK Connect's Kepala Cabang never gets MKH Property access; this only adds
-- read for the CFO's existing pengajuan queue, one-way sync.
-- ============================================================================
...
begin
  ...
  begin
    ...
    -- (utang toko) input from MK Connect's construction finance module.
    elsif p_event_type = 'construction_expense_submitted' then
      v_tipe := case p_payload ->> 'expense_type'
        when 'gaji_tukang' then 'tukang'
        when 'pembelian_material' then 'bahan'
        else 'bahan'
      end;
      ...
      v_data := jsonb_build_object(
        'source', 'mk_connect',
        'nominal', v_nominal,
        'tgl', coalesce(p_payload ->> 'expense_date', to_char(current_date, 'YYYY-MM-DD')),
        'proyek_nama', coalesce(p_payload ->> 'branch_name', 'Kendari') || ' - Pembangunan Cabang',
        'nama', p_payload ->> 'party_name',
        'description', p_payload ->> 'description',
        'metode_bayar', p_payload ->> 'payment_method',
        'branch_name', p_payload ->> 'branch_name',
        'input_by', p_payload ->> 'created_by_name'
      );
...
comment on function public.sync_inbound is
  'PostgREST-callable receiver for MK Connect -> MKH Property sync events (CRM payments, payroll, commission, bonus, reimbursement, HR expenses, construction project expenses). Authenticated via X-Sync-Secret header against Supabase Vault, not JWT.';
