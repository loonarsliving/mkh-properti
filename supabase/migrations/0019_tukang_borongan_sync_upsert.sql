-- Owner's request: Jogja's running Loonars project already uses this real
-- gaji-tukang/borongan flow (pengajuan/tukang_borongan/jurnal) day to day --
-- "gaji tukang" and "upah borongan tukang" are the same thing in the real
-- business, tukang is treated as borongan. Rather than migrating the
-- running project onto MK Connect's newer WBS-weighted cm_labor_contracts
-- model (which posts to MK Connect's own construction_expenses, not
-- mkh-properti's jurnal), the owner will type each blok's current sisa
-- kontrak / terbayar straight into a new MK Connect form and have it
-- pushed here one-way. No mirror/read-sync back to MK Connect is wanted.
--
-- tukang_borongan has no unique constraint beyond its pkey (confirmed),
-- so this branch matches an existing contract by proyek + blok + nama
-- (case-insensitive/trimmed) and updates it, or inserts a new row if none
-- matches yet.
--
-- CREATE OR REPLACE below is the full live definition of sync_inbound as
-- of migration 0018, with only one new "elsif" branch added right before
-- the final "else" (unknown event_type) branch, plus one new declare var.
