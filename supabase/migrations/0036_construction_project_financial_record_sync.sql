-- ============================================================================
-- MKH Property — 0036: sync_inbound — construction_project_financial_record
--
-- New event type from MK Connect's Loonars Coffee construction pilot
-- (Construction Project Control Core): fires once, when a
-- construction_cost_requests row reaches POSTED (owner approved, transfer
-- confirmed, money actually recorded) — see MK Connect migration
-- 0256_loonars_coffee_construction_project.sql's
-- trg_construction_cost_request_sync trigger. Writes BOTH:
--   1. construction_project_financial_records (0035) — the dedicated
--      project-control record with full detail (RAB item, cost code,
--      requester/approver, WA request id).
--   2. jurnal — the real ledger entry, under proyek 'LL' (Loonars
--      Jogja/Living's own project code, per the owner's decision that
--      Loonars Coffee's money lives in those same books), same
--      debit/credit convention construction_expense_submitted already uses
--      for material vs labor. `jurnal_no` links the two records so this
--      is never double-counted against the general finance table.
--
-- This is the ONLY delivery path available for an MK-Connect-originated
-- event (MK Connect's sync_dispatch_pending always POSTs to this one
-- sync_inbound RPC, regardless of event_type — see MK Connect's
-- supabase/migrations/0059_sync_dispatch_rpc_target.sql), so — unlike
-- Villa's deliberately separate villa_income_sync RPC (0033), which is
-- called by an entirely different, third-party system — this really does
-- have to be a new branch of the shared sync_inbound function rather than
-- a second isolated RPC. Every other branch of sync_inbound is
-- UNCHANGED — this migration is exactly 0029's body plus one new elsif
-- (CREATE OR REPLACE needs the full body, per this function's own
...
begin
  ...
  begin
    if p_event_type = 'crm_payment_approved' then
    ...
    elsif p_event_type = 'finance_expense_transfer_rejected' then
      select * into v_pay from public.pengajuan where id = (p_payload ->> 'pengajuan_id')::bigint for update;
      if not found then
        raise exception 'pengajuan % not found', p_payload ->> 'pengajuan_id';
      end if;
      ...
      update public.pengajuan
        set status = 'rejected',
            verified_by = coalesce(p_payload ->> 'rejected_by', 'Super Admin (WA)'),
            data = v_pay.data || jsonb_build_object('super_admin_reject_reason', p_payload ->> 'reason'),
            updated_at = now()
        where id = v_pay.id;

      v_target_ref := v_pay.id::text;

    elsif p_event_type = 'material_expense_receipt_submitted' then
      v_proyek := coalesce(nullif(p_payload ->> 'proyek', ''), 'LL');
      v_nominal := coalesce((p_payload ->> 'nominal')::numeric, 0);
      if v_nominal <= 0 then
        raise exception 'material_expense_receipt_submitted requires a positive nominal (got %)', p_payload ->> 'nominal';
      end if;
      v_tgl := coalesce(nullif(p_payload ->> 'tgl', '')::date, current_date);
      v_ket := coalesce(nullif(p_payload ->> 'keterangan', ''), p_payload ->> 'item', 'Nota WhatsApp (dibaca AI)');
      ...
      v_data := jsonb_build_object(
        'tgl', v_tgl::text,
        'no', v_no,
        'jenis', coalesce(nullif(p_payload ->> 'jenis_akun', ''), '5-1001'),
        'akunNama', coalesce(nullif(p_payload ->> 'jenis_nama', ''), 'Pembelian Material'),
        'rek', coalesce(nullif(v_rekening, ''), '1-1001'),
        'rekNama', case when v_rekening is not null and v_rekening <> '' then trim(both from ('Bank ' || coalesce(v_bank, ''))) else 'Kas Tunai' end,
        'item', p_payload ->> 'item',
        'items', coalesce(p_payload -> 'items', jsonb_build_array(jsonb_build_object('nama', p_payload ->> 'item', 'nilai', v_nominal))),
        'nominal', v_nominal,
        'ket', v_ket,
        'keterangan', v_ket,
        'proyek_nama', coalesce(nullif(p_payload ->> 'proyek_nama', ''), v_proyek),
        'pengawas', p_payload ->> 'pengawas',
        'sumber', coalesce(nullif(p_payload ->> 'sumber', ''), 'whatsapp_ai'),
        'ai_supplier', p_payload ->> 'supplier',
        'ai_notes', p_payload ->> 'ai_notes'
      );

      insert into public.pengajuan (proyek, tipe, data, status, created_by)
      values (
        v_proyek, 'bahan', v_data, 'pending',
        coalesce(nullif(p_payload ->> 'created_by_label', ''), 'Pengawas: ' || coalesce(p_payload ->> 'pengawas', 'WhatsApp AI'))
      )
      returning id into v_pengajuan_id;
    ...
    elsif p_event_type = 'construction_project_financial_record' then
      -- This migration. See 0035 (construction_project_financial_records)
      -- and MK Connect's 0256 (trg_construction_cost_request_sync) for the
      -- source side.
      v_proyek := coalesce(nullif(p_payload ->> 'mkh_project_code', ''), 'LL');
      v_nominal := coalesce((p_payload ->> 'amount')::numeric, 0);
      if v_nominal <= 0 then
        raise exception 'construction_project_financial_record requires a positive amount (got %)', p_payload ->> 'amount';
      end if;
      v_tgl := current_date;
      v_tipe := p_payload ->> 'request_type';

      if v_tipe = 'contractor_payment' then
        v_debit_akun := '5-1003'; v_debit_nama := 'Biaya Upah Tukang';
      else
        v_debit_akun := '5-1001'; v_debit_nama := 'Pembelian Material';
      end if;
      ...
      v_ket := 'Loonars Coffee: ' || coalesce(p_payload ->> 'description', '-')
        || case when coalesce(p_payload ->> 'party_name', '') <> '' then ' - ' || (p_payload ->> 'party_name') else '' end
        || ' - Diajukan: ' || coalesce(p_payload ->> 'requester_name', '-')
        || ' - Disetujui: ' || coalesce(p_payload ->> 'approver_name', '-');

      insert into public.jurnal (tgl, no, ket, akun, nama, proyek, d, k, idempotency_key, source_system, source_ref) values
        (v_tgl, v_no, v_ket, v_debit_akun, v_debit_nama, v_proyek, v_nominal, 0, p_idempotency_key || ':d', 'mk_connect', p_payload ->> 'cost_request_id'),
        (v_tgl, v_no, v_ket, v_credit_akun, v_credit_nama, v_proyek, 0, v_nominal, p_idempotency_key || ':k', 'mk_connect', p_payload ->> 'cost_request_id');

      insert into public.construction_project_financial_records (
        proyek, construction_project_code, mkc_cost_request_id, cost_code, transaction_type, description,
        amount, vendor_payee, requester, approver, payment_status, transaction_date, reference_document,
        wa_request_id, jurnal_no, idempotency_key
      ) values (
        v_proyek, coalesce(p_payload ->> 'construction_project_code', 'LOONARS_COFFEE'),
        (p_payload ->> 'cost_request_id')::uuid, p_payload ->> 'cost_code', v_tipe, coalesce(p_payload ->> 'description', '-'),
        v_nominal, p_payload ->> 'party_name', p_payload ->> 'requester_name', p_payload ->> 'approver_name',
        'posted', v_tgl, coalesce(p_payload ->> 'posted_expense_id', p_payload ->> 'posted_labor_payment_id'),
        p_payload ->> 'wa_request_id', v_no, p_idempotency_key
      )
      on conflict (idempotency_key) do nothing
      returning id into v_cpfr_id;
...
comment on function public.sync_inbound is
  'PostgREST-callable receiver for MK Connect -> MKH Property sync events (crm_payment_approved, payroll_run_approved, sales_commission_approved, payroll_salary_generated, bonus_approved, reimbursement_approved, hr_expense_approved, construction_expense_submitted, construction_fund_transfer_recorded, finance_expense_transfer_confirmed, finance_expense_transfer_rejected, material_expense_receipt_submitted, loonars_closing_verified, loonars_closing_rejected, loonars_fee_requested, loonars_fee_wa_decision, tukang_borongan_sisa_upsert, construction_project_financial_record). Authenticated via X-Sync-Secret header against Supabase Vault, not JWT. construction_project_financial_record (0036) is Loonars Coffee''s Construction Project Control Core pilot — posts both a dedicated construction_project_financial_records row (0035) and the matching jurnal entry under proyek=LL, one canonical idempotency_key linking both so it is never double-counted.';
