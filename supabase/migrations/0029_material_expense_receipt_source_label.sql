-- ============================================================================
-- MKH Property — 0029: let `sumber` reflect the real origin of a
-- material_expense_receipt_submitted pengajuan
--
-- Owner's ask: Anang can now request a fund advance directly on WhatsApp
-- (MK Connect's contractor-fund-request.ts), in his own words -- not just
-- report spending against an existing advance. It reuses this same
-- sync_inbound event since the resulting pengajuan is identical in shape
-- to an AI-read nota, but data.sumber was hardcoded to 'whatsapp_ai',
-- which would mislabel a contractor-initiated fund request as if it came
-- from a nota photo. Now takes whatever the caller sends (defaulting to
-- 'whatsapp_ai' for backward compatibility with the existing nota flow).
-- Every other branch of sync_inbound is unchanged (CREATE OR REPLACE needs
-- the full body).
-- ============================================================================
...
begin
  ...
  begin
    if p_event_type = 'crm_payment_approved' then
      ...
      -- Endy/Rebecca sending a nota (receipt) photo captioned "nota" on
      -- WhatsApp. MK Connect's AI already read the item/nominal off the
      -- photo (no image reaches this function or gets stored anywhere --
      -- see MK Connect's lib/ai/domains/material-receipt-submission.ts).
      -- Builds the exact same pengajuan.data shape lapor-pengeluaran.html /
      -- lapor-biaya-lain.html produce, and deliberately does NOT set
      -- source_system='mk_connect' so trg_pengajuan_expense_submitted_sync
      -- (0008) still fires and notifies the branch's Kepala Cabang (Vando)
      -- by WhatsApp -- same verification step a manual submission goes
      -- through, before Super Admin transfers the money.
...
comment on function public.sync_inbound is
  'PostgREST-callable receiver for MK Connect -> MKH Property sync events (crm_payment_approved, payroll_run_approved, sales_commission_approved, payroll_salary_generated, bonus_approved, reimbursement_approved, hr_expense_approved, construction_expense_submitted, construction_fund_transfer_recorded, finance_expense_transfer_confirmed, finance_expense_transfer_rejected, material_expense_receipt_submitted, loonars_closing_verified, loonars_closing_rejected, loonars_fee_requested, loonars_fee_wa_decision, tukang_borongan_sisa_upsert). Authenticated via X-Sync-Secret header against Supabase Vault, not JWT. material_expense_receipt_submitted (0027, itemized per 0028, sumber label per 0029) creates a bahan pengajuan either from an AI-read WhatsApp nota photo (Endy/Rebecca) or a contractor-initiated fund request (Anang) -- no photo is stored, and source_system is deliberately left unset so the branch Kepala Cabang still gets notified by WhatsApp exactly like a manual submission.';
