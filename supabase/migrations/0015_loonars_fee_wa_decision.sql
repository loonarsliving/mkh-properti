-- ============================================================================
-- MKH Property — 0015: handle loonars_fee_wa_decision (temporary WhatsApp
-- approval for fee claims, companion to MK Connect's
-- lib/ai/domains/loonars-fee-approval.ts + 0186 migration)
--
-- While CFO approval on /verifikasi.html isn't staffed, a Super Admin can
-- approve a pending loonars fee claim by replying "ya" on WhatsApp. MK
-- Connect enqueues loonars_fee_wa_decision here instead of the browser
-- PATCHing `pengajuan` directly -- so this branch has to do BOTH things the
-- CFO's own "Setujui" click does: post the commission journal entry
-- (verifikasi.html's client-side SYNC_COA.komisi logic, mirrored here in
-- SQL) and flip `pengajuan.status`. That UPDATE re-triggers the existing
-- pengajuan_komisi_decided_sync trigger (0014), which mirrors the decision
-- onto loonars_fee and fires loonars_closing_approved (target sync) +
-- loonars_fee_decided (WhatsApp receipt) exactly as it does for a normal
-- CFO approval -- no need to duplicate that part here.
--
-- Guarded by checking loonars_fee.status = 'pending' first (row-locked) so a
-- race with a real CFO click on /verifikasi.html can't double-post the
-- journal; the outer sync_inbound idempotency_key check already prevents a
-- retried webhook call from doing it twice.
-- ============================================================================
  ...
  begin
    if p_event_type = 'crm_payment_approved' then
      ...
      if v_fee.status <> 'pending' then
        -- Already decided (e.g. CFO clicked Setujui on /verifikasi.html first) -- no-op, not an error.
        ...
        -- pengajuan_komisi_decided_sync (0014) fires on the UPDATE above and
        -- mirrors the decision onto loonars_fee + enqueues
        -- loonars_closing_approved/loonars_fee_decided -- not duplicated here.
        v_target_ref := v_fee_id::text;
      end if;
...
comment on function public.sync_inbound is
  'PostgREST-callable receiver for MK Connect -> MKH Property sync events (crm_payment_approved, payroll_run_approved, sales_commission_approved, payroll_salary_generated, bonus_approved, reimbursement_approved, hr_expense_approved, loonars_closing_verified, loonars_closing_rejected, loonars_fee_requested, loonars_fee_wa_decision). Authenticated via X-Sync-Secret header against Supabase Vault, not JWT. loonars_fee_wa_decision is a temporary path for Super Admin approving a fee claim by WhatsApp reply instead of the CFO clicking Setujui on /verifikasi.html.';
