-- ============================================================================
-- MKH Property — 0027: WhatsApp nota-photo -> pengajuan (material_expense_receipt_submitted)
--
-- Owner's ask: instead of Endy/Rebecca filling in lapor-pengeluaran.html /
-- lapor-biaya-lain.html by hand, they can send a nota (receipt) photo
-- captioned "nota" to the system's WhatsApp number. MK Connect's AI reads
-- the nominal/item off the photo (no image is stored -- read once, then
-- discarded) and calls this new inbound event instead. The resulting
-- pengajuan is created exactly like a manual bahan submission: same
-- tipe='bahan', same data shape lapor-pengeluaran.html/lapor-biaya-lain.html
-- use, and deliberately WITHOUT source_system='mk_connect' so it still
-- fires trg_pengajuan_expense_submitted_sync (0008) and notifies that
-- branch's Kepala Cabang (Vando) via WhatsApp -- exactly the same
-- verification step a manual submission goes through before Super Admin
-- transfers the money. Every other branch of sync_inbound is unchanged
  ...
  begin
    if p_event_type = 'crm_payment_approved' then
      ...
      v_data := jsonb_build_object(
        ...
        'item', p_payload ->> 'item',
        'items', jsonb_build_array(jsonb_build_object('nama', p_payload ->> 'item', 'nilai', v_nominal)),
        'nominal', v_nominal,
        'ket', v_ket,
        'keterangan', v_ket,
