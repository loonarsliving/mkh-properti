-- ============================================================================
-- MKH Property — 0028: itemized breakdown for material_expense_receipt_submitted
--
-- Owner's feedback after trying 0027: the AI reply/report was showing one
-- combined item description with a single price, not each nota line item
-- with its own price. MK Connect (expense-receipt-recognition.ts) now reads
-- every line item separately and sends them as a real `items` array
-- (`[{"nama":..., "nilai":...}]`) in the payload. This just makes
-- pengajuan.data.items pass that array straight through instead of
-- synthesizing a single-entry array from the flattened item/nominal
-- fields -- item/nominal (the joined-text summary and the total) are
-- unchanged and still used as-is for backward compatibility with any
-- caller that doesn't send `items`. Every other branch of sync_inbound is
