-- Companion to mkhsistem's 0225: money coming INTO a construction project
-- (HQ funding it, via the manual dialog or the WA "kirim bukti trf ke
-- Fasly/Kendari" flow) now posts a real jurnal entry here too, not just
-- expenses going out. Debit Kas Tunai (project now has more cash on hand),
-- credit Modal dari Kantor Pusat (3-1001, a fresh equity account -- an
-- internal capital contribution from HQ to fund the project, distinct
-- from 3-1002 Laba Ditahan which is already in use).
--
-- CREATE OR REPLACE below is the full live definition of sync_inbound as
-- of migration 0019, with only one new "elsif" branch added right before
-- the final "else" (unknown event_type) branch. Every other branch is
-- copied verbatim.
