-- ============================================================================
-- MKH Property — 0018: Jurnal/kas for gaji tukang & pembelian bahan now
-- posts on TRANSFER CONFIRMATION, not on Kepala Cabang approval
--
-- Owner's request: today, jurnal (and branch kas) is deducted the instant
-- Kepala Cabang approves a pengajuan in verifikasi.html -- before Super
-- Admin has actually transferred any money. Super Admin then does the real
-- bank transfer separately and forwards a screenshot over WhatsApp as an
-- informal, out-of-system confirmation. That's backwards for cash-basis
-- accounting: the books say money is gone before it necessarily is.
--
-- New flow: Kepala Cabang's approve() (verifikasi.html) now ONLY flips
-- pengajuan.status to 'approved' -- no jurnal/bayar_tukang/tukang_borongan
-- writes happen there anymore. Those move to a new inbound sync event,
-- finance_expense_transfer_confirmed, fired by MK Connect the moment Super
-- Admin sends the actual bukti transfer photo over WhatsApp (AI-read there,
-- forwarded on to Kepala Cabang + Endy so they can pass it to the tukang/
-- toko -- see MK Connect's lib/ai/domains/transfer-proof-confirmation.ts).
--
-- Also extends the existing finance_expense_approved outbound payload with
-- branch_name/item/no (mirroring finance_expense_submitted, 0008) so MK
-- Connect can queue the "awaiting transfer" entry and later match the
-- WhatsApp reply back to the right pengajuan/branch.
-- ============================================================================
...
comment on function public.pengajuan_expense_approved_sync is
  'Enqueues an outbound finance_expense_approved sync event to MK Connect when an admin-submitted expense (bahan/tukang) is approved by Kepala Cabang -- notifies every super_admin AND (0018) queues the pengajuan as awaiting transfer proof, no jurnal posted yet.';

-- ----------------------------------------------------------------------------
-- sync_inbound: new finance_expense_transfer_confirmed branch. CREATE OR
-- REPLACE of the full function (every other branch copied verbatim from the
-- live definition, per the 0016 incident's rule: pg_get_functiondef the
-- live version first, never hand-reconstruct from migration history alone).
-- ----------------------------------------------------------------------------
...
begin
  ...
  begin
    if p_event_type = 'crm_payment_approved' then
      ...
      -- Owner's request (0018): jurnal for a human-submitted gaji tukang /
      -- pembelian bahan pengajuan now posts HERE, on Super Admin's WhatsApp
      -- bukti transfer confirmation -- not at Kepala Cabang approval time.
