-- ============================================================================
-- MKH Property — 0017: Post construction_expense_submitted directly to jurnal
--
-- 0016 routed Kepala Cabang gaji tukang / pembelian material (utang toko)
-- input through the CFO approval queue (pengajuan), like a human-entered
-- expense. Owner's explicit correction: this should NOT require CFO
-- approval -- it's a report of money already spent/owed by the branch, so
-- it posts DIRECTLY to jurnal, same pattern as crm_payment_approved
-- (0002/0003) which posts Incoming Cash directly with no approval gate.
--
-- Chart of accounts used (both already exist except 2-1003, which this
-- migration effectively introduces as a new liability account -- COA here
-- is free text, no schema for it):
--   - gaji_tukang:        Dr 5-1003 Biaya Upah Tukang / Cr 1-1001 Kas Tunai
--   - pembelian_material:  Dr 5-1001 Pembelian Material / Cr 2-1003 Utang Toko Bangunan
--     (always credited here, never cash -- MK Connect's construction_expenses
--     only syncs pembelian_material as payment_method='utang', per its own
--     0193 design: "semua material toko bangunan dibeli secara utang").
-- ============================================================================
...
begin
  ...
  begin
    ...
    -- This migration: Kepala Cabang gaji tukang / pembelian material
    -- (utang toko) input from MK Connect. Posts DIRECTLY to jurnal -- no
    -- CFO approval gate, this is a report of money already spent/owed by
    -- the branch, not a request for money.
...
comment on function public.sync_inbound is
  'PostgREST-callable receiver for MK Connect -> MKH Property sync events (CRM payments, payroll, commission, bonus, reimbursement, HR expenses; construction project expenses post directly, no approval gate). Authenticated via X-Sync-Secret header against Supabase Vault, not JWT.';
