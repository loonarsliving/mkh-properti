...
## Loonars Villa ("villa" repo, separate Supabase project — added 2026-09-10)
- **Purpose**: Automatic monthly sync of Loonars Villa rental income (rental/cafe/spa/lainnya) into this app's `pendapatan_villa` report table (migration 0032). This is a **different business/system** from "loonars-sales" above — Loonars Villa is the rental-operations app (bookings, investor revenue share), not the unit-siteplan-sales app.
- **Location in code**: migration `0033_villa_income_sync.sql` (function `public.villa_income_sync(p_periode date, p_items jsonb)`).
- **Data IN (to this app)**: once a month, villa's own `villa-api` Edge Function (in the separate `svcmybsziaelwwdrnzcv` Supabase project) computes `gross_revenue` (rental) + `walkin_income` (cafe/spa/lainnya) via its existing `computeReport()`/`computeWalkinIncome()`, then calls `POST /rest/v1/rpc/villa_income_sync` here with `p_periode` (first-of-month date) and `p_items` (`[{kategori, jumlah}, ...]`).
- **Data OUT**: none — one-directional, villa → mkh-properti only.
- **Authentication method**: deliberately NOT the service-role-key model `sync_inbound` uses for MK Connect (that would mean handing this project's service_role key to a second external company's system — too broad a blast radius). Instead: PostgREST's normal anon-key RPC path, with the function itself checking a dedicated Vault secret (`villa_sync_shared_secret`) against the `x-villa-sync-secret` request header. `EXECUTE` on this one function is granted to `anon`/`authenticated`; the secret is what actually gates it. Secret value is not committed anywhere (Vault only), same convention as `mk_sync_shared_secret`.
- **Idempotency**: `idempotency_key = 'villa:<YYYY-MM>:<kategori>'` — a retried/duplicate monthly sync upserts in place, never duplicates. Logged to `sync_log` (`event_type='villa_income_reported'`) same as MK Connect events, for CFO-visible audit trail.
- **Status**: DONE (RPC live in production, tested 2026-09-10 with a `set_config('request.headers', ...)` simulation of the header PostgREST normally sets — real end-to-end villa→mkh-properti traffic not yet observed). Villa-side caller (`villa-api`'s `POST /cron/sync-mkh-income` + a new Vercel Cron in the `villa` repo) built in the same work session — see that repo's own INTEGRATIONS.md/ARCHITECTURE.md for the outbound half.
- **Why not `sync_inbound`**: that function is documented shared production infrastructure for MK Connect specifically ("CREATE OR REPLACE needs the full body", "a bad edit here has broken multiple downstream flows before") — adding an unrelated external system's event type to it would risk that code path for no benefit. A small dedicated function is safer and independently auditable.
